...
const CampaignUpdate = () => {
  ...
  const handleAddNewCampaign = async (values) => {
    ...
    try {
      //   await axios.put(`${apiURL}/campaigns/${queryParamValue}`, {
      //     ...newValues,
      //     content,
      //     user,
      //   });
      dispatch(CampUpdate({ ...newValues, content, user, id }));
      toast.success("Update Campaign successfully");
      setContent("");
      reset({});
    } catch (error) {
      toast.error("Can't update new Campaign");
    }
  };
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        //   `${apiURL}/campaigns/${queryParamValue}`
        // );

        dispatch(getdataCampWithParam(id));

        // setdataCamp(response.data);
      } catch (error) {
        console.log(
          "🚀 ~ file: CampaignUpdate.js:93 ~ useEffect ~ error:",
          error
        );
      }
    }
  ...
  useEffect(() => {
    setValue("title", dataCamp.title);
  }, [dataCamp, setValue]);
  useEffect(() => {
    setValue("category", dataCamp?.category);
    // console.log(dataCamp.category);
    setLabelCategory(dataCamp?.category?.name);
  }, [dataCamp, setValue]);
